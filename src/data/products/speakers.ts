...
export const speakers = async (): Promise<Product[]> => {
  const CACHE_KEY = "speaker_products_session";
  
  // Kiểm tra môi trường để tránh lỗi ReferenceError: sessionStorage is not defined
  ...
  if (isBrowser) {
    ...
    if (cachedData) {
      console.log("🔊 [Speaker] Lấy từ Session Storage (0 Read)");
      return JSON.parse(cachedData) as Product[];
    }
  }

  // 2. Nếu không có hoặc đang ở Server, mới gọi Firebase
  try {
    const q = query(
    ...
    // 3. Lưu vào Session cho lần sau (Chỉ thực hiện ở Client)
    if (isBrowser) {
    ...
    console.error("Lỗi khi fetch speakers:", error);
    return [];
  }
};
